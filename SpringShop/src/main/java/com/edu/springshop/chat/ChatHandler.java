...
//웹소켓 요청을 처리할 핸들러
//TextWebSocketHandler : 이미지, 동영상, 텍스트 다 가능하지만 우리의 목적은 채팅이기 때문에 text~~기반의 핸들러를 재정의한다
public class ChatHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		logger.info("클라이언트가 보낸 메세지 "+msg);
		
		//접속한 모든 사용자에게 메세지 보내기(브로드 캐스팅)
		TextMessage data = new TextMessage("서버에서 보낸 말"+message.getPayload()); 
